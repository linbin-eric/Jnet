...
public abstract class Chunk<T>
{
    ...
    public boolean apply(int need, Handler<T> bucket)
    {
        if (pool[1] < need)
        {
            return false;
        }
        // int selectedLevel = size.length - 1;
        // for (; selectedLevel > -1; selectedLevel--)
        // {
        // if (size[selectedLevel] >= need)
        // {
        // break;
        // }
        // }
        ...
        for (int i = start; i <= end; i++)
        {
            if (pool[i] == bucketSize && isAvailable(i, bucketSize))
            {
                ...
                initHandler(bucket, i, off, bucketSize);
                return true;
            }
        }
        return false;
    ...
    protected abstract void initHandler(Handler<T> bucket, int index, int off, int len);
