...
public class ThreadAttchConfiguration implements Configuration
{
    ...
    public ThreadAttchConfiguration(final ExecutorService executorService, AioListener aioListener, FrameDecodec frameDecodec, StreamProcessor[] inProcessors, StreamProcessor[] outProcessors, int maxMerge, AsynchronousSocketChannel socketChannel, SendBufStorage sendBufStorage, ByteBuf<?> inCachedBuf, ByteBuf<?> outCachedBuf)
    {
        ...
        readProcessor = new ReadProcessor() {
            ...
            public void process(ByteBuf<?> buf, SendBufStorage bufStorage, StreamProcessor[] inProcessors, ChannelContext channelContext) throws Throwable
            {
                ...
                if (processor == null)
                {
                    processor = new ThreadAttachProcessor();
