...
@Slf4j
public class DefaultWriteCompleteHandler2 extends AtomicInteger implements WriteCompletionHandler
{
    protected static final long                      STATE_OFFSET   = UNSAFE.getFieldOffset("state", DefaultWriteCompleteHandler2.class);
    ...
    public DefaultWriteCompleteHandler2(Pipeline pipeline)
    {
    ...
    private void writeQueuedBuffer()
    {
        try
        {
            ...
            if (sendingData == null)
            {
                sendingData = allocator.ioBuffer(Math.min(maxWriteBytes, queueCapacity.get()));
            }
